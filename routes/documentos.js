...
const {getDocumentos, saveDocuments, getDocumentosbyAdmisionista, getDocumentobyCode, updateEstadoDocumento, getSiniestrobyId, showDocument } = require("../Request/documentos");
...
route.get("/getDocumentoBase64", async (req, res) => {
  try {
    const {documento} = req.query
    const result = await get(showDocument(documento))
    res.send(result);
...
route.post("/FirmarDocumentos", async (req, res) => {
  try{
    ...
    res.send(apiResponse(result, 200));
  }catch(error){
    res.send(apiResponse([], 500, error));
  }
});
...
route.get("/infoSiniestro", async (req, res) => {
  try {
    const {id} = req.query
    const result = await get(getSiniestrobyId(id))
    res.send(result);
